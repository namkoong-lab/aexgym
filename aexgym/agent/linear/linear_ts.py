...
class LinearTS(LinearAgent):
    ...
    def forward(self, beta, sigma, contexts=None,  action_contexts = None, objective = None, costs=None):
        ...
        fake_rewards = objective(fake_mc)
        if self.constraint:
        ...
        ones = torch.ones((batch_size, self.n_samples), dtype=int, device=contexts.device)
        rows = torch.arange(batch_size, dtype=int, device=contexts.device).unsqueeze(1)
        rows = (rows*ones).flatten()
        counts = torch.zeros((batch_size, self.model.n_arms), dtype=int, device = contexts.device)
        probs = counts.index_put_((rows, indices), torch.ones_like(indices, dtype=int, device=contexts.device), accumulate=True) / self.n_samples
        if self.toptwo == True:
            probs = torch.clamp(probs, min = 0.0000001, max = 0.9999999) / torch.sum(probs, dim=1, keepdim=True)
            ...
            mask = mask.unsqueeze(0).to(contexts.device)

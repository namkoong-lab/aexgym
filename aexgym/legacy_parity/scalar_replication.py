...
def run_trial(config: PaperScalarConfig, seed: int, policies: list[LegacyPolicy]) -> dict:
    ...
    for policy in policies:
        ...
        trial["policies"][policy.name] = {
            ...
            "selected_true_best": bool(selected_arm == int(np.argmax(theta))),
...
def aggregate_trials(trials: list[dict], policies: list[LegacyPolicy]) -> dict:
    ...
    for policy in policies:
        ...
        selected_true_best = np.array([trial["policies"][policy.name]["selected_true_best"] for trial in trials], dtype=float)
        out[policy.name] = {

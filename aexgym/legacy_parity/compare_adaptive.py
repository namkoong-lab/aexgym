...
from aexgym.legacy_parity.scalar_replication import (
    PaperScalarConfig,
    load_config,
    make_policies,
    paper_priors,
...
def compare_config(config: PaperScalarConfig, adaptive_root: str | Path) -> dict:
    ...
    for seed in range(config.seed, config.seed + config.n_runs):
        ...
        for name in config.policies:
            ...
            new_selected_true_best = float(current["policies"][name]["selected_true_best"])
            rows.append(
                {
                    "seed": seed,
                    "policy": name,
                    ...
                    "old_selected_true_best": old_correct,
                    "new_selected_true_best": new_selected_true_best,
                    "selected_true_best_diff": new_selected_true_best - old_correct,
                }
            )

    max_abs_regret_diff = max((abs(row["regret_diff"]) for row in rows), default=0.0)
    max_abs_selected_true_best_diff = max((abs(row["selected_true_best_diff"]) for row in rows), default=0.0)
    return {
        "config": asdict(config),
        "adaptive_root": str(Path(adaptive_root).resolve()),
        "max_abs_regret_diff": max_abs_regret_diff,
        "max_abs_selected_true_best_diff": max_abs_selected_true_best_diff,
        "all_match": max_abs_regret_diff <= 1e-12 and max_abs_selected_true_best_diff <= 1e-12,
        "rows": rows,
    }
